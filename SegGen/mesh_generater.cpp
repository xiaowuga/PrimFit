...
namespace QuadFit {
    ...
    void Mesh_Generater::set_bbox(const easy3d::Box3 &bbox_) {
        bbox = bbox_;
        double len = bbox.diagonal_length() * bbox_eps;
            ...
            if(type == QuadFit::SurfaceType::PLANE) {
                ...
                for(size_t j = 0; j < shapes.size(); j++) {
                    ...
                    if(tmp == QuadFit::SurfaceType::PLANE
                       && shapes[j]->distance(p) - r < 0.002) {
                        tangent_planes.emplace_back(shapes[j]);
                    }
                }
                generate_cylinder_mesh(shapes[i], tangent_planes, proxy_points[i], proxy_indices[i]);
...
//        }
        ...
        for (std::size_t i = 0; i < tangent_planes.size(); i++) {
            ...
            for (int j = 0; j < n1; j++) {
                double d1 = std::abs(plane->distance(points[j]));
                double d2 = std::abs(plane->distance(points[j + n2 * n1]));
                if ((d1 + d2) / 2 < len * 0.002) {
                    points[j] = p2 + radius * 1.01 * cos(j * 2 * M_PI / n1) * base1
                                + 1.01 * radius * sin(j * 2 * M_PI / n1) * base2;
                    for(int k = 1; k <= n2; k++) {
                        points[j + k * n1] = points[j] + 1.0 * k * h / n2 * auv;
                    }
                }
...
//            indices.emplace_back((i + 1) % n);
//            indices.emplace_back(i + n);
//            indices.emplace_back(i + n);
//            indices.emplace_back((i + 1) % n);
//            indices.emplace_back((i + 1) % n + n);
//        }
        return true;
    }
...
//                indices[((i-1)*2*n_latitude + j)] << (i-1)*n_latitude+j,
