...
namespace QuadFit {
    namespace Util {
        void load_data(std::string& primitive_path,
            ...
            std::ifstream in_seg(primitive_path);
            json js_seg;
            in_seg >> js_seg;
            int num_seg = js_seg.size();
            segments.clear();segments.resize(num_seg);
            for(int i = 1; i <= num_seg; i++) {
                json tmp = js_seg["Primitive"+std::to_string(i)];
                int x = tmp.size();
                for(size_t j = 0; j < tmp.size(); j++) {
                    int idx = tmp[j].get<int>() - 1;
                    segments[i - 1].emplace_back(idx);
                }
            }

            std::ifstream in_para(parametric_ptah);
            json js_para;
            in_para >> js_para;
            int num_primitive = js_para.size();
            for(int i = 1; i <= num_primitive; i++) {
                json tmp = js_para["Primitive"+std::to_string(i)];
                std::string type = tmp["type"].get<std::string>();
                json para = tmp["param"];
                std::vector<easy3d::vec3> pp;
                for(int j = 0, k = 0; j < para.size(); j += 3, k++) {
                    easy3d::vec3 v = easy3d::vec3(
                            para[j + 0].get<float>()
                            ,para[j + 1].get<float>()
                                    ,para[j + 2].get<float>());
                    pp.emplace_back(v);
                ...
                if(type == "Plane") {
                    SurfaceParameters p;
                    p.pos = pp[2];
                    p.dir = easy3d::cross(pp[0], pp[1]).normalize();
                    p.r1 = -easy3d::dot(p.pos, p.dir);
                    p.r2 = 0;
                    shapes.emplace_back(construct_bytype(SurfaceType::PLANE, p));
                }
                else if(type == "Cylinder") {
                    SurfaceParameters p;
                    p.pos = pp[3];
                    p.dir = easy3d::normalize(pp[2]);
                    p.r1 =  easy3d::norm(pp[0]);
                    p.r2 =  0;
                    shapes.emplace_back(construct_bytype(SurfaceType::CYLINDER, p));
                }
                else if(type == "Cone"){
                    SurfaceParameters p;
                    p.pos = pp[5];
                    p.dir = easy3d::normalize(pp[4]);
                    p.r1 = std::atan(easy3d::norm(pp[0]+pp[2]));
                    p.r2 = 0;
                    shapes.emplace_back(construct_bytype(SurfaceType::CONE, p));
                }
                else if(type == "Sphere") {
                    SurfaceParameters p;
                    p.pos = pp[3];
                    p.dir = easy3d::vec3(0,0,0);
                    p.r1 = easy3d::norm(pp[2]);
                    p.r2 = 0;
                    shapes.emplace_back(construct_bytype(SurfaceType::SPHERE, p));
                }
                else if(type == "Torus") {
                    SurfaceParameters p;
                    p.pos = pp[5];
                    p.dir = easy3d::cross(pp[0], pp[1]).normalize();
                    p.r1 = easy3d::norm(pp[0]);
                    p.r2 = easy3d::norm(pp[2]);
                    shapes.emplace_back(construct_bytype(SurfaceType::TORUS, p));
                }
                shapes.back()->setColor(color_table[(i - 1) % 15]);
            }
            std::vector<int> label;

            std::vector<easy3d::io::Element> elements;
            easy3d::io::PlyReader reader;
            if (!reader.read(cloud_path, elements)) {
                std::cout << "can not load cloud!" << std::endl;
            }
            for(int i = 0; i < elements.size(); i++) {
                if(elements[i].name == "vertex") {
                    easy3d::io::Vec3Property vertex;
                    IO::details::extract_named_property(elements[i].vec3_properties, vertex, "point");
                    label.resize(vertex.size(), -1); points.resize(vertex.size());normals.resize(vertex.size());
                    for(size_t j = 0; j < segments.size(); j++) {
                        for(size_t k = 0; k < segments[j].size(); k++) {
                            int idx = segments[j][k];
                            label[idx] = j;
                        }
                    }
                    for(size_t j = 0; j < vertex.size(); j++) {
                        ...
                        int idx = label[j];
                        if(idx != -1) {
                            normals[j] = shapes[idx]->normal(points[j]);
                        }
                    }
                    break;
                }
            ...
            for(auto& item : mp) {
                ...
                for(auto& jtem : item.second) {
                    ...
                    if(patch_indices.size() == 1) {
                        ...
                        std::cout << "Error, there is a egde around by more than 4 patch." << std::endl;
                    }
                }
            }
