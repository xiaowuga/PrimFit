...
namespace PrimFit {
    ...
    public:
        ...
        int find_root(int k) {
            return bcj[k] == k ? k : bcj[k] = find_root(bcj[k]);
        }

    public:
        std::vector<bool> patch_state;
        std::vector<int> bcj;
        std::vector<int> patch_map2variable; // map each to variable
        std::vector<int> curve_map2variable;
        std::vector<int> sharp_curve_map2variable;
