...
namespace PrimFit {
    ...
    bool Mesh_Generater::generate_proxy_mesh(std::vector<SurfacePrimitive *> &shapes,
        ...
        for(int i = 0; i < num; i++) {
            PrimFit::SurfaceType type = shapes[i]->getType();
            if(type == PrimFit::SurfaceType::PLANE) {
                generate_plane_mesh(shapes[i], proxy_points[i], proxy_indices[i]);
            } else if(type == PrimFit::SurfaceType::CYLINDER) {
                PrimFit::SurfaceParameters para = shapes[i]->getParameters();
                vec3 p = para.pos;
                vec3 d = para.dir;
                double r = para.r1;
...
//                std::cout << "cylinder"<<i <<std::endl;
                for(size_t j = 0; j < shapes.size(); j++) {
                    ...
                    PrimFit::SurfaceType tmp = shapes[j]->getType();
                    if(tmp == PrimFit::SurfaceType::PLANE) {
...
//                } else {
                    generate_cylinder_mesh(shapes[i], tangent_planes, proxy_points[i], proxy_indices[i], 36, 5);
//                }
            } else if(type == PrimFit::SurfaceType::CONE) {
                std::vector<PrimFit::SurfacePrimitive*> tangent_planes;
                generate_cone_mesh(shapes[i], tangent_planes, proxy_points[i], proxy_indices[i]);
            } else if(type == PrimFit::SurfaceType::SPHERE) {
                std::vector<PrimFit::SurfacePrimitive*> tangent_planes;
//                if(i == 31) {
...
//                }
            } else if(type == PrimFit::SurfaceType::TORUS) {
                std::vector<PrimFit::SurfacePrimitive*> tangent_planes;
